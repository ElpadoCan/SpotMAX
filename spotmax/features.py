...
def get_aggregating_spots_feature_func():
    func = {
        ...
        'num_spots': ('x', 'count', 0),
        'spotfit_sum_foregr_integral': ('I_foregr', 'sum', np.nan),
        'spotfit_sum_tot_integral': ('I_tot', 'sum', np.nan),
        'mean_sigma_z': ('sigma_z_fit', 'mean', np.nan),
        'mean_sigma_y': ('sigma_y_fit', 'mean', np.nan),
        'mean_sigma_x': ('sigma_x_fit', 'mean', np.nan),
        'std_sigma_z': ('sigma_z_fit', 'std', np.nan),
        'std_sigma_y': ('sigma_y_fit', 'std', np.nan),
        'std_sigma_x': ('sigma_x_fit', 'std', np.nan),
        'sum_A_fit': ('A_fit', 'sum', np.nan),
        'mean_B_fit': ('B_fit', 'mean', np.nan),
        'solution_found': ('solution_found', 'mean', np.nan),
        'mean_reduced_chisq': ('reduced_chisq', 'mean', np.nan),
        'combined_p_chisq': ('p_chisq', scipy.stats.combine_pvalues, np.nan),
        'mean_RMSE': ('RMSE', 'mean', np.nan),
        'mean_NRMSE': ('NRMSE', 'mean', np.nan),
        'mean_F_NRMSE': ('F_NRMSE', 'mean', np.nan),
        'mean_ks': ('KS_stat', 'mean', np.nan),
        'combined_p_ks': ('p_KS', 'mean', np.nan),
        'mean_ks_null': ('null_ks_test', 'mean', np.nan),
        'mean_chisq_null': ('null_chisq_test', 'mean', np.nan),
        'mean_QC_passed': ('QC_passed', 'mean', np.nan)
    }
    return func
